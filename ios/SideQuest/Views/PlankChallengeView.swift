...
struct PlankChallengeView: View {
    let quest: Quest
    let instanceId: String
    let appState: AppState
    @Environment(\.dismiss) private var dismiss
    @State private var cameraService = PlankCameraService()
    @State private var phase: PlankPhase = .setup
    ...
    private var targetHold: TimeInterval { quest.targetHoldSeconds ?? 60 }
    ...
    private var borderColor: Color {
        ...
        if cameraService.displayStanding { return .red }
        if cameraService.displayKneesOnGround { return .red }
        if cameraService.displayPlankDetected { return .green }
        return .orange
    }
    ...
    private var hasActiveWarning: Bool {
        phase == .active && (
            ...
            !cameraService.displayPlankDetected
        )
    }
    ...
    private var mainContent: some View {
        ...
        .onChange(of: cameraService.displayPlankDetected) { _, inPlank in
            guard phase == .active, cameraService.cameraAvailable else { return }
            withAnimation(.easeOut(duration: 0.15)) {
                borderGlowOpacity = inPlank ? 0.5 : 0.8
            }
        }
    ...
    private var standingWarningOverlay: some View {
        ZStack {
            ...
            VStack(spacing: 20) {
                ...
                Text("Get down into plank position\nto continue tracking")
    ...
    private var setupInstructions: some View {
        VStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                instructionRow(icon: "iphone.gen3", text: "Prop phone upright at ground level")
                instructionRow(icon: "figure.core.training", text: "Get into plank position facing camera")
                instructionRow(icon: "arrow.left.and.right", text: "Make sure full body is visible")
                instructionRow(icon: "knee", text: "Keep knees off the ground")
                instructionRow(icon: "timer", text: "Timer only counts while holding plank")
            }
        }
        .padding(16)
    ...
    private var countdownOverlay: some View {
        ZStack {
            ...
            VStack(spacing: 16) {
                ...
                Text("Hold plank when countdown ends")
                    .font(.subheadline)
    ...
    private var activeWarningBanners: some View {
        VStack(spacing: 8) {
            ...
            if !cameraService.displayPlankDetected && cameraService.displayBodyDetected && !cameraService.displayStanding {
                largeWarningBanner(
    ...
    private var formBadge: some View {
        HStack(spacing: 4) {
            Image(systemName: cameraService.displayPlankDetected ? "checkmark.circle.fill" : "xmark.circle.fill")
                .font(.caption2)
            Text("Plank")
                .font(.caption2.weight(.semibold))
        }
        .foregroundStyle(cameraService.displayPlankDetected ? .green : .red)
        .padding(.horizontal, 8)
        .padding(.vertical, 5)
        .background(.black.opacity(0.5), in: Capsule())
        .animation(.easeInOut(duration: 0.3), value: cameraService.displayPlankDetected)
    }
    ...
    private var cameraUnavailablePlaceholder: some View {
        VStack(spacing: 24) {
            ...
            Button {
                ...
                Label("Start Plank", systemImage: "play.fill")
    ...
    private func startTimer() {
        ...
        timer = Timer.scheduledTimer(withTimeInterval: 0.5, repeats: true) { _ in
            Task { @MainActor in
                ...
                if cameraService.cameraAvailable {
                    if cameraService.displayPlankDetected && !cameraService.displayKneesOnGround && !cameraService.displayStanding && cameraService.displayBodyDetected {
...
nonisolated enum PlankPhase: Equatable {
    case setup
    case active
}
